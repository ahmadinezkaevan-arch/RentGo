...
const stats: {label:string;value:string;note:string;icon:IconName;tone:string}[] = [
  ...
  {label:"Pemesanan aktif",value:"12",note:"4 mulai hari ini",icon:"calendar",tone:"bg-[#eaf1ff] text-[#245fd4]"},
  {label:"Menunggu tindakan",value:"8",note:"perlu ditinjau",icon:"clock",tone:"bg-[#eaf1ff] text-[#245fd4]"},
  {label:"Pendapatan bulan ini",value:"Rp 38,4 jt",note:"+12,8% dari Agustus",icon:"trend",tone:"bg-[#eaf1ff] text-[#245fd4]"},
];
const tasks: {title:string;detail:string;count:string;tone:string;icon:IconName;href:string}[] = [
  {title:"Verifikasi dokumen",detail:"5 pelanggan menunggu pemeriksaan",count:"05",tone:"bg-[#eaf1ff] text-[#245fd4]",icon:"shield",href:"/admin/verifikasi"},
  ...
  {title:"Periksa pembayaran",detail:"2 bukti bayar baru masuk",count:"02",tone:"bg-[#eaf1ff] text-[#245fd4]",icon:"wallet",href:"/admin/transaksi"},
];
const bookings = [
  {id:"BKG-0428",name:"Budi Santoso",initials:"BS",vehicle:"Toyota Avanza",dates:"16-19 Sep 2026",total:"Rp 997.500",status:"Menunggu verifikasi",tone:"bg-[#eaf1ff] text-[#245fd4]"},
  ...
  {id:"BKG-0426",name:"Rizky Pratama",initials:"RP",vehicle:"Honda Vario 160",dates:"15-20 Sep 2026",total:"Rp 570.000",status:"Sedang disewa",tone:"bg-[#eaf1ff] text-[#245fd4]"},
...
const schedule = [
  ...
  {time:"11:00",title:"Pengembalian Honda Brio",person:"Dimas Ardi / BKG-0419",dot:"bg-[#245fd4]"},
  {time:"15:30",title:"Pengambilan Honda Vario",person:"Rizky Pratama / BKG-0426",dot:"bg-[#2563eb]"},
];

function Sidebar() {
  return <aside className="fixed inset-y-0 left-0 z-30 hidden w-[260px] flex-col border-r border-[#e5eaf1] bg-white text-[#17263b] lg:flex">
    <div className="flex h-[76px] items-center gap-3 border-b border-[#e5eaf1] px-7"><Image src="/referensi/Logo%20RentGo.svg" alt="RentGo" width={174} height={58} className="h-5 w-auto" /><span className="ml-1 rounded bg-[#eaf1ff] px-2 py-1 text-[10px] font-semibold text-[#245fd4]">Admin</span></div>
    <nav className="flex-1 px-4 py-6" aria-label="Navigasi admin"><p className="mb-3 px-3 text-xs font-medium text-[#7a8799]">Menu utama</p><div className="space-y-1">{nav.map((item,index)=><Link key={item.label} href={item.href} aria-current={index===0?"page":undefined} className={"flex h-11 items-center gap-3 rounded-lg px-3 text-sm font-medium "+(index===0?"bg-[#2f6ce5] text-white shadow-[0_8px_24px_rgba(47,108,229,.3)]":"text-[#526177] hover:bg-[#eef4ff] hover:text-[#245fd4]")}><Icon name={item.icon} className="h-[19px] w-[19px]"/><span>{item.label}</span>{item.count?<span className="ml-auto min-w-6 rounded-full bg-[#eaf1ff] px-1.5 py-0.5 text-center text-[11px] text-[#245fd4]">{item.count}</span>:null}</Link>)}</div></nav>
    <div className="border-t border-[#e5eaf1] p-4"><div className="flex items-center gap-3 rounded-xl bg-[#f7f9fc] p-3"><span className="grid h-10 w-10 place-items-center rounded-full bg-[#dce8ff] text-sm font-bold text-[#2053ad]">RA</span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">Raka Admin</span><span className="block truncate text-xs text-[#899dbd]">admin@rentgo.id</span></span><button type="button" aria-label="Keluar" className="text-[#899dbd] hover:text-[#245fd4]"><Icon name="logout"/></button></div></div>
  </aside>;
}
function Header() {
  return <header className="sticky top-0 z-20 flex h-[76px] items-center justify-between border-b border-[#e6eaf0] bg-white/95 px-5 backdrop-blur sm:px-8 lg:ml-[260px] lg:px-9">
    <Link href="/admin" className="flex items-center gap-2 text-[#0d1f3c] lg:hidden"><Image src="/referensi/Logo%20RentGo.svg" alt="RentGo" width={174} height={58} className="h-5 w-auto" /></Link>
...
export default function AdminDashboard() {
  return <div className="min-h-screen bg-white text-[#17263b]"><Sidebar/><Header/>
    <main className="pb-24 lg:ml-[260px] lg:pb-10"><div className="mx-auto max-w-[1500px] px-5 py-7 sm:px-8 lg:px-9 lg:py-8">
      ...
      <section className="mt-5 grid gap-5 xl:grid-cols-[1.45fr_.8fr]">
        <div className="overflow-hidden rounded-xl border border-[#e1e6ec] bg-white"><div className="flex items-start justify-between border-b border-[#edf0f3] px-5 py-[18px] sm:items-center sm:px-6"><div><h2 className="font-bold">Perlu ditangani</h2><p className="mt-1 text-xs text-[#66758a]">Tugas prioritas agar operasional tetap lancar.</p></div><span className="inline-flex items-center gap-1.5 rounded-full bg-[#eaf1ff] px-2.5 py-1 text-xs font-semibold text-[#245fd4]"><span className="h-1.5 w-1.5 rounded-full bg-[#245fd4]"/>8 tugas</span></div>
        ...
        <aside className="rounded-xl border border-[#e1e6ec] bg-white p-5 text-[#17263b] sm:p-6"><div className="flex items-center justify-between"><div><h2 className="font-bold">Agenda hari ini</h2><p className="mt-1 text-xs text-[#66758a]">3 jadwal / 14 September</p></div><span className="grid h-9 w-9 place-items-center rounded-lg bg-[#eaf1ff] text-[#245fd4]"><Icon name="calendar"/></span></div><ol className="mt-6 space-y-5">{schedule.map((item,index)=><li key={item.time} className="grid grid-cols-[45px_14px_1fr] gap-2.5"><time className="pt-0.5 text-xs font-semibold text-[#526177]">{item.time}</time><span className="relative flex justify-center"><span className={"mt-1.5 h-2.5 w-2.5 rounded-full ring-[3px] ring-white/10 "+item.dot}/>{index<schedule.length-1?<span className="absolute bottom-[-22px] top-4 w-px bg-[#dbe7fb]"/>:null}</span><span><span className="block text-sm font-semibold leading-5">{item.title}</span><span className="mt-1 block text-xs text-[#66758a]">{item.person}</span></span></li>)}</ol><Link href="/admin/pemesanan" className="mt-6 flex items-center justify-center gap-2 rounded-lg border border-[#c9daf7] bg-[#eef4ff] py-2.5 text-xs font-semibold text-[#245fd4]">Buka jadwal lengkap <Icon name="arrow" className="h-3.5 w-3.5"/></Link></aside>
      </section>
