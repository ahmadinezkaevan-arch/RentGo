...
export async function updateBookingStatus(formData: FormData) { await requireAdmin(); const id=text(formData,"id"),status=text(formData,"status"); if(!id||!["PENDING_PAYMENT","AWAITING_VERIFICATION","CONFIRMED","ACTIVE","COMPLETED","CANCELLED","REJECTED"].includes(status)) fail("/admin/pemesanan","invalid-booking"); await prisma.booking.update({where:{id},data:{status:status as never}}); ["/admin","/admin/pemesanan","/admin/laporan","/profile"].forEach((path) => revalidatePath(path)); }
export async function updateDocumentStatus(formData: FormData) { const admin=await requireAdmin(),id=text(formData,"id"),status=text(formData,"status"); if(!id||!["VERIFIED","REJECTED"].includes(status)) fail("/admin/verifikasi","invalid-document"); await prisma.identityDoc.update({where:{id},data:{status:status as never,reviewedBy:admin.id,reviewedAt:new Date()}}); ["/admin","/admin/verifikasi","/admin/pelanggan","/profile"].forEach((path) => revalidatePath(path)); }
export async function toggleCustomerActive(formData: FormData) { await requireAdmin(); const id=text(formData,"id"),active=text(formData,"active")==="true"; if(!id) fail("/admin/pelanggan","invalid-customer"); await prisma.user.update({where:{id},data:{isActive:active}}); revalidatePath("/admin/pelanggan"); }